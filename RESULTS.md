...
### 8f. Continuous-burst movie on held-out data (the "does it actually work" figure)

Take a **held-out validation burst file** (10 seconds at 120 ms cadence = 83 frames), mask each
frame with the same synthetic wedge (25 % of bins), run the U-Net on every frame, and play it
back as a side-by-side animation: TRUE skymap vs. MASKED input vs. U-Net IMPUTED, plus a
running plasma-density panel below.

- `outputs/unet_full_3rounds/inpainting_movie.gif`     — the 10 s animation (≈ 8 fps)
- `outputs/unet_full_3rounds/inpainting_keyframes.png` — 6 evenly-spaced stills, for the paper

Headline over the full 10 s burst: **median \|Δn\|/n = 0.206, mean = 0.200** (consistent with
the broader-validation §8e numbers, on data the trainer literally never saw). Visually the
...
(θ, φ) plane and rerun **pure inference**, no retraining (figure:
`outputs/unet_full_3rounds/monte_carlo.png`, right panel; notebook
`notebooks/08_monte_carlo.ipynb`).

| Mask shift from the trained position | U-Net vs. baseline (masked-MSE ratio) |
|---|---|
| 0 (default) | **10.4× better** |
| φ +8 | 1.3× better |
| φ +16 (half the azimuth range) | **23.6× better** |
| θ +4 | 8.2× better |
| θ +8 (mask now in the originally-visible half) | *0.97×* (model fails, ties baseline) |
| θ +4 *and* φ +16 | **27× better** |

The model generalises across φ — even half the azimuth range — because at fixed energy the
distribution is largely rotationally symmetric in φ (the spacecraft spins in φ), so it learned
the *task* of inpainting, not a φ-specific lookup. It **breaks on a large θ shift** that moves
the mask into the previously-visible half of θ, which the model has never seen occluded. The
fix is straightforward and is item #1 in `NEXT_STEPS.md`: train with **randomised mask
positions** (or the realistic `explosion.npy` energy-dependent mask).

## 8h. Is this enough for a paper?

Honest answer: **not yet for a top-tier methods paper, but close enough to be a workshop /
short / preprint paper now, and a clear road to a real one.**

What we have:
- A debugged, end-to-end pipeline (data → model → training → resume → evaluation).
- A 3-D U-Net that beats interpolation by ~10× in masked-MSE and ~2.5× in density (~20 %
  median fractional density error) on **held-out** data.
- A stable streaming-training procedure that survives crashes (validated in anger).
- A model zoo (temporal channels, B-field features, physics head, ConvLSTM, JEPA-lite).
- The "does it actually work" movie on a 10-second held-out burst.
- An honest mask-generalisation analysis exposing one real weakness (θ-position dependence).

What's missing before a top venue (JGR: Space Physics / Earth & Space Science):
1. **Data diversity** — 26 burst files from 2 days of MMS1, very likely one plasma regime.
   A reviewer will want multiple regimes (solar wind, magnetosheath, magnetopause, plasma
   sheet, tail) and all four MMS spacecraft. `download_cdfs.py` can do this; the issue is GPU
   hours.
2. **Realistic mask** — repeat the headline with `--mask explosion` (energy-dependent, the
   actual spacecraft-body occlusion) and train with randomised mask positions so the
   θ-generalisation failure goes away.
3. **A baseline that's actually used in the literature** — energy-shell-mean is a sanity floor;
   need to compare against e.g. Gaussian processes / Kriging or any published gap-fill
   technique in space physics.
4. **A real science demonstration** — show the reconstruction preserves a specific known
   feature (a reconnection signature, an electron beam, a heat-flux event). "It inpaints well"
   isn't a result; "it lets us recover the moments through a known event the gap would have
   otherwise hidden" is.
5. **Statistical rigor** — bootstrap test set, confidence intervals on the headline numbers,
   formal ablation table (plain / + temporal / + B-field / + physics head).
6. **L2 moments cross-check** — validate against the official `des-moms` product, not only our
   own `Skymap`-computed moments.

Realistic targets:
- **AGU / EGU poster, right now**: yes, with the current results and an honest "limitations"
  slide. Probably the best way to get community feedback before the bigger investment.
- **Workshop paper / preprint** (e.g. ML4PS at NeurIPS, AGU short paper): also yes, today,
  framed as "method + first results, scaling to the full archive in progress."
- **Methods paper in JGR or Earth & Space Science**: items 1–5 first. Doable, ~3–6 months on a
  GPU.
- **Top-tier science paper (Nature / Science)**: needs item 4 to be genuinely novel science
  enabled by the method — e.g. detecting reconnection events through gaps that would otherwise
  hide them.
